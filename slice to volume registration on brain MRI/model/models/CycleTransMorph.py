...
import models.SpatialTransformer
...
class linear_predict(nn.Module):
    def __init__(self, num_layers=4, img_size=(192, 192, 192),embed_dim=96, same_sub=False, hidden_features=1296,act_layer=nn.GELU, drop=0.):
        super().__init__()
        out_features = 6 if same_sub else 9
        img_size_pre = list(i*(2**(-num_layers-1)) for i in img_size)
        embed_dim_pre = embed_dim*(2**(num_layers-1))
        in_features = np.prod(img_size_pre) * embed_dim_pre
        self.act = act_layer()
        self.drop = nn.Dropout(drop)
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, img_size=(192, 192, 192), patch_size=4, in_chans=2, reg_head_chan=16,
                 embed_dim=96, depths=[2, 2, 2, 2], depths_decoder=[1, 2, 2, 2], num_heads=[3, 6, 12, 24],
                 window_size=(6, 6, 6), mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1, predict_drop_rate=0.,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
                 use_checkpoint=False, same_sub=True, **kwargs):
        super().__init__()
        ...
        self.predict = linear_predict(num_layers=len(depths), img_size=img_size,embed_dim=embed_dim,
                                        same_sub=same_sub,act_layer=nn.GELU, drop=predict_drop_rate) 
...
class CycleTransMorph(LoadableModel):
    @store_config_args
    def __init__(self, config):
        '''
        CycleTransMorph Model
        '''
        super(CycleTransMorph, self).__init__()
    ...
    def forward(self,x):
        ...
        return x
        # x_in_mf = torch.cat((moving, fixed), dim=1)
        # x_in_fm = torch.cat((fixed, moving), dim=1)

        # #################################################
        # # Moving to fixed
        # #################################################
        # x_mf = self.transformer(x_in_mf)
        # flow_mf = self.reg_head(x_mf)
 
        # pos_flow_mf = flow_mf

        # pos_flow_mf = self.resize(pos_flow_mf)

        # preint_flow_mf = pos_flow_mf

        # # negate flow for bidirectional model
        # neg_flow_mf = -1 * pos_flow_mf

        # # integrate to produce diffeomorphic warp
        # pos_flow_mf = self.integrate(pos_flow_mf)
        # neg_flow_mf = self.integrate(neg_flow_mf)

        # pos_flow_mf = self.fullsize(pos_flow_mf)
        # neg_flow_mf = self.fullsize(neg_flow_mf)

        # # warp image with flow field
        # warp_mf = self.spatial_trans(moving, pos_flow_mf)
        # warp_inv_mf = self.spatial_trans(warp_mf, neg_flow_mf)

        # #################################################
        # # fixed to moving
        # #################################################      
        # x_fm = self.transformer(x_in_fm)
        # flow_fm = self.reg_head(x_fm)

        # pos_flow_fm = flow_fm

        # pos_flow_fm = self.resize(pos_flow_fm)

        # preint_flow_fm = pos_flow_fm

        # # negate flow for bidirectional model
        # neg_flow_fm = -1 * pos_flow_fm

        # # integrate to produce diffeomorphic warp
        # pos_flow_fm = self.integrate(pos_flow_fm)
        # neg_flow_fm = self.integrate(neg_flow_fm)

        # pos_flow_fm = self.fullsize(pos_flow_fm)
        # neg_flow_fm = self.fullsize(neg_flow_fm)

        # # warp image with flow field
        # warp_fm = self.spatial_trans(fixed, pos_flow_fm)
        # warp_inv_fm = self.spatial_trans(warp_fm, neg_flow_fm)

        # return (warp_mf, pos_flow_mf, warp_inv_mf, neg_flow_mf), (warp_fm, pos_flow_fm, warp_inv_fm, neg_flow_fm)
